...
export default function GooeyNav({
    ...
    const activate = (liEl: HTMLElement, index: number, href: string) => {
        ...
        // Let the gooey animation play, then navigate via the router.
        window.setTimeout(() => navigate(href), animationTime)
    }

    const handleClick = (e: React.MouseEvent<HTMLLIElement>, index: number, href: string) => {
        e.preventDefault()
        activate(e.currentTarget, index, href)
    }
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLAnchorElement>, index: number, href: string) => {
        if (e.key === 'Enter' || e.key === ' ') {
            ...
            if (liEl) activate(liEl, index, href)
        }
    }
